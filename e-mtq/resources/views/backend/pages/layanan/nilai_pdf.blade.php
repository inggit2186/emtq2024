	...
	<table class='table table-bordered'>
		...
		<tbody>
			...
			<tr>
				...
				@endif
				@if($cmtq->penanya == 1)
					@php $penanya = App\Models\Nilai::where(['peserta' => $peserta->nama, 'status' => 0])->first(); @endphp
					@if(!empty($penanya))
				...
				@foreach($bidang3 as $bidang3)
					@php $nilai = App\Models\Nilai::with(['cmtq','gmtq','bidang','peserta'])->where(['peserta' => $peserta->nama,'status' => 0, 'bidang_id' => $bidang3->id])->get(); @endphp
			...
			<tr style="background-color:#00F2F2">
				...
				@endif
				@if($cmtq->penanya == 1)
					@php $penanya = App\Models\Nilai::where('peserta',$peserta2->nama)->first(); @endphp
					@if(!empty($penanya))
					<td>{{$penanya->penanya}}</td>
					@endif
				@endif
				...
				@php $bidang3 = App\Models\Bidang::with('cmtq')->where('cat_id',$cmtq->id)->get(); @endphp
				@foreach($bidang3 as $bidang3)
					@php $nilai2 = App\Models\Nilai::with(['cmtq','gmtq','bidang','peserta'])->where(['peserta' => $peserta2->nama,'bidang_id' => $bidang3->id])->get(); @endphp
