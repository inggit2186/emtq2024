...
<body>
  <div class="container">
	...
	@php
		$a=1;
		if($cmtq->id == 7){
		$peserta2 = App\Models\Peserta::with(['cmtq','gmtq'])->where('golongan_id',$gmtq2->id)->orderBy('total','DESC')->get()->sortBy('nomor');
		}else{
		$peserta2 = App\Models\Peserta::with(['cmtq','gmtq'])->where('golongan_id',$gmtq2->id)->orderBy('total', 'DESC')->get();
		}
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <table id="table1" class="table table-striped table-bordered">
          ...
          <tbody>
			...
			@php 
				$nilai = App\Models\Nilai::with(['cmtq','gmtq','bidang','peserta'])->where(['peserta' => $peserta->nama, 'status' => 0])->get();
			@endphp
			@if($peserta->jk == "Putra")
			<tr>
				<td>{{$i}}</td>
				...
				@endforeach
				<td>{{$peserta->total}}</td>
				<td><a href="{{route('edit.rank', Crypt::Encrypt($peserta->id))}}" class="badge bg-warning" style="font-size: 15px; color:black;" onclick="modal()">EDIT</a> &nbsp; <a href="{{route('delete.rank', Crypt::Encrypt($peserta->id))}}" class="badge bg-danger" onclick="return confirm('Anda Yakin Untuk Menghapus?')" style="font-size: 15px; color:black;">DELETE</a></td>
			<?php $i++; ?>
			</tr>
			@endif
			@endforeach
			...
			@php 
				$nilai2 = App\Models\Nilai::with(['cmtq','gmtq','bidang','peserta'])->where(['peserta' => $peserta2->nama, 'status' => 0])->get();
			@endphp
			...
			<tr style="background-color:#00B6B6">
				...
				<td><a href="{{route('edit.rank', Crypt::Encrypt($peserta2->id))}}" class="badge bg-warning" style="font-size: 15px; color:black;" onclick="modal()">EDIT</a> &nbsp; <a href="{{route('delete.rank', Crypt::Encrypt($peserta2->id))}}" class="badge bg-danger" onclick="return confirm('Anda Yakin Untuk Menghapus?')" style="font-size: 15px; color:black;">DELETE</a></td>
			<?php $a++; ?>
			</tr>
			@endif
			@endforeach
